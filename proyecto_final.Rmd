...
library(psych) # Libreria para hacer Factor de Analisis
...
setwd("C:/Users/diego/Documents/Ingeniería_Ambiental/Diplomado/Modulo_3/Proyecto_M3/diplo-modulo3-equipo-11/Data")
...
Como no se proporcionó un diccionario del significado de las variables nosotros lo creamos investigando un poco del deporte: 
...
filtrado_muestreo_base <- muestreo_field %>% 
   dplyr::select(GS, InnOuts, PB, SB,CS)
...
filtrado_muestreo_base <- nombres %>% 
   dplyr::select(-deathYear, -deathMonth, -deathDay, -birthYear, -birthMonth, -birthDay, -deathCountry, -deathState, -deathCity, -deathDate)
...
filtrado_muestreo_base <- salonFama %>% 
   dplyr::select(ballots, needed, votes, needed_note)
...
bateo_ligas <- bateo |>
  inner_join(field |> dplyr::select(playerID, lgID_field = lgID), by = "playerID") |>
...
bateo_fisico <- promedios_bateo |>
  inner_join(nombres |> dplyr::select(playerID, height, weight), by = "playerID") |>
...
```
Para responder esta pregunta y determinar qué tipo de jugador gana más, realizamos un modelo de regresión que incluye tanto el tipo de jugador como el total de juegos disputados, con el objetivo de hacer la comparación más justa.

Al aplicar una prueba de hipótesis para evaluar si, en promedio, los lanzadores ganan más que los bateadores (ajustando por el número total de juegos), obtuvimos un **p-valor = 0.0871**.  
Esto indica que **no hay suficiente evidencia estadística** para afirmar que los lanzadores ganan más, por lo que la diferencia entre ambos grupos **no es significativa**.
...
En esta gráfica podemos observar que, en promedio, los lanzadores ganan más que los bateadores por el mismo tiempo de juego.  
Sin embargo, como su carrera suele ser más corta, esto podría deberse a que la posición de lanzador es más lesiva, especialmente en la zona del hombro, lo que puede acortar la duración de su trayectoria profesional.  
Además, dado que pocos jugadores pueden lanzar a gran velocidad, esta escasez podría encarecer la posición.
...
Para responder esta pregunta utilizaremos regresion logistica, para tratar de calcular la probabilidad de que un jugador entre al salon de la fama.


```{r}

data_fama <- salonFama %>%
  left_join(field, by = "playerID") %>%
  left_join(pitcheo, by = "playerID") %>%
  mutate(tipo_jugador = ifelse(POS == "P", "Lanzador", "Bateador")) %>%
  mutate(tipo_jugador = as.factor(tipo_jugador)) %>%
  dplyr::select(playerID, yearID=yearID.x, inducted, tipo_jugador, InnOuts, PO, HR, SO ) %>%
  mutate( #Filtrando los datos, para no tener NAs
    InnOuts = ifelse(is.na(InnOuts), 0, InnOuts),
    PO = ifelse(is.na(PO), 0, PO),
    HR = ifelse(is.na(HR), 0, HR),
    SO = ifelse(is.na(SO), 0, SO)
  ) %>%
  group_by(playerID) 
...
```{r}
filtroFamosos <- data_fama %>%
   filter(!is.na(tipo_jugador) | !is.na(inducted)) %>%
  group_by(playerID) %>%
  summarise(
    fue_selc = any(inducted == "Y"),  # Si fue seleccionado
    año = ifelse(fue_selc, min(yearID[inducted == "Y"]), max(yearID)),
    tipo_jugador = tipo_jugador[yearID == año][1],  # toma la posición en ese año
    InnOuts = sum(InnOuts, na.rm = TRUE),
    PO = sum(PO, na.rm = TRUE),
    HR = sum(HR, na.rm = TRUE),
    SO = sum(SO, na.rm = TRUE), 
    fue_selc = ifelse(fue_selc, "Y", "N"),  # Cambiamos a Y o N
  ) %>%
  mutate(
    fue_selc = as.factor(fue_selc)
  ) 
...
```{r}
modloGLM <- glm(fue_selc ~ tipo_jugador  + PO + HR + SO, data = filtroFamosos, family = binomial(link = "logit"))

summary(modloGLM)
```


En base al modelo propuesto, podemos observar que los jugadores cuya posición principal es la de lanzador presentan, en promedio, una menor probabilidad de ser incluidos en el Salón de la Fama, en comparación con los bateadores.

Asimismo, el modelo muestra que los jugadores con mayores valores de PO (putouts) y SO (strikeouts) tienen una mayor probabilidad estimada de ser inducidos, mientras que un mayor número de HR (home runs permitidos) se asocia negativamente con la inducción.

Esto sugiere que los jugadores que participan más activamente en jugadas defensivas o que dominan desde el montículo con ponches, tienden a ser mejor valorados en la elección al Salón de la Fama que aquellos que permiten más carreras.


Veamos unas graficas para corroborar esto:

```{r}
# Quitando los NAs para que no nos hagan ruido a la hora de graficar
limpios_fama <- na.omit(filtroFamosos)

```
```{r}
limpios_fama %>%
 filter(InnOuts >= 0L & InnOuts <= 5127792L) %>%
 ggplot() +
  aes(x = PO, fill = fue_selc) +
  geom_histogram(bins = 30L) +
  scale_fill_manual(values = c(N = "#D21F13", Y = "#0D93B3")) +
  labs(title = "PO") +
  theme_minimal() +
  theme(
  ...
  facet_wrap(vars(tipo_jugador))
```


Aquí podemos ver reflejada la tendencia observada en el modelo: la probabilidad de ser inducido al Salón de la Fama aumenta si el jugador era bateador y si tiene un mayor número de *PO*.
...
limpios_fama %>%
 ...
 ggplot() +
  ...
  facet_wrap(vars(tipo_jugador))
```
De la misma forma, observamos que un beisbolista tiene una mayor probabilidad de entrar al Salón de la Fama si es bateador y si su número de *home runs* (HR) es mayor.
...
Por último, en esta gráfica observamos que los bateadores con un mayor número de *SO* (strikeouts) tienen una mayor probabilidad de entrar al Salón de la Fama.
...
```{r}
numeroBat <- filtroFamosos %>%
  filter(tipo_jugador == "Bateador") %>%
  summarise(
    total = n(),
    saleccionado = sum(fue_selc == "Y"),
    proporcion = saleccionado / total
  )
...
numLanzador <- filtroFamosos %>%
  filter(tipo_jugador == "Lanzador") %>%
  summarise(
    total = n(),
    noselec = sum(fue_selc == "Y"),
    proporcion = noselec / total
  )
...
```{r}
ggplot(limpios_fama) +
  aes(x = tipo_jugador, fill = fue_selc) +
  geom_bar() +
  scale_fill_manual(values = c(N = "#CF2D21", Y = "#1A7EAE")) +
  labs(title = "Bateadores vs Lanzadores") +
  theme_minimal() +
  theme(axis.text.x = element_text(size = 15L))

```


Para responder esta pregunta, utilizaremos los datos que ya hemos filtrado y procesado en el ejercicio anterior.

Podemos observar que, en el caso de los bateadores, hay un total de **939 jugadores nominados**, de los cuales **211** lograron entrar al Salón de la Fama. Esto representa una proporción del **22.47%**.  
En el caso de los lanzadores, se cuenta con **440 jugadores nominados**, de los cuales **74** fueron inducidos, lo que corresponde a una proporción del **16.81%**.

Por lo tanto, podemos concluir que es más probable ser nominado y aceptado al Salón de la Fama si se es **bateador** que si se es **lanzador**.  
Esto podría deberse a que se valora más la parte ofensiva del juego que la defensiva, ya que los bateadores suelen protagonizar las jugadas más espectaculares y llamativas —como los *home runs*—, las cuales generan mayor emoción en el público y atraen a los espectadores. En contraste, los lanzadores no suelen realizar jugadas tan vistosas, por lo que podrían ser percibidos como menos atractivos para la audiencia.



## ¿El número de home runs aumenta las probabilidades de ingresar al Salón de la Fama?

Como observamos en nuestro modelo de regresión logística, tener un mayor número de *home runs* no aumenta significativamente la probabilidad de entrar al Salón de la Fama, ya que no es uno de los factores más determinantes según el análisis estadístico.
...
ggplot(limpios_fama) +
  aes(x = HR, fill = fue_selc) +
  geom_histogram(bins = 21L) +
  scale_fill_manual(values = c(N = "#E32215", Y = "#1DB8EC")) +
 labs(
  x = "Número de HR",
  y = "Número de jugadores",
  title = "Número de HR vs si fueron seleccionados"
) +
theme_minimal()
...
En esta gráfica podemos corroborar que realizar un mayor número de **HR** no necesariamente aumenta las probabilidades de ser seleccionado para entrar al Salón de la Fama.  
Observamos casos donde algunos jugadores hicieron más de **500** *home runs* y aun así no fueron inducidos, lo que sugiere que existen otros factores que los jueces consideran más importantes.
...
```{r}
limpios_fama <- limpios_fama %>% #Haciendo un diltrado por decadas
  mutate(decada = paste0(floor(año / 10) * 10, "s"))


proporcion_decada <- limpios_fama %>%
  group_by(decada) %>%
  summarise(
    total = n(),
    seleccionados = sum(fue_selc == "Y"),
    proporcion = seleccionados / total
  )
...
```{r}
ggplot(proporcion_decada) +
  aes(x = decada, y = proporcion) +
  geom_col(fill = "#16539E") +
  labs(
    x = "Decadas",
    y = "Proporcion",
    title = "Proporcion de seleccion al salon de la fama"
  ) +
  ggthemes::theme_pander() +
  theme(
...
Así es, en esta gráfica podemos observar cómo la proporción de selección al Salón de la Fama ha disminuido desde la década de los 70, cuando en esa decada en concreto más del **50%** de los jugadores nominados eran aceptados.  
En contraste, en la década del **2020**, esta proporción ha caído a cerca del **10%**.



```{r}
ggplot(limpios_fama) +
  aes(x = decada, fill = fue_selc) +
  geom_bar() +
  scale_fill_manual(values = c(N = "#F44132", Y = "#3695C1")) +
  labs(title = "Décadas de selección al Salón de la Fama") +
  theme_minimal()

```

En esta otra gráfica podemos observar que el número de jugadores que ingresaron al Salón de la Fama entre las décadas de **1940** y **2010** se ha mantenido relativamente estable, con entre **30** y **40** seleccionados por década.  
Sin embargo, lo que reduce la probabilidad de ser aceptado es el número de candidatos postulados. Por ello, la década en la que fue más fácil ser seleccionado fue en los **70s** y en las décadas anteriores.
...
```{r}
filterNames <- nombres %>%
  dplyr::select(playerID, debut) %>%
  dplyr::mutate(debut = as.Date(debut, format = "%Y-%m-%d"),
         debute_años = year(debut)) #seleccionar solo el año del debut

```
```{r}
#Uniendo los datos
union_fama <- left_join(filterNames, limpios_fama, by = "playerID") %>%
  filter(!is.na(fue_selc)) %>%
  dplyr::select(playerID, debute_años, fue_selc, año ) %>%
  mutate(
    difefencia = año - debute_años
  )
...
```{r}
union_fama %>%
 filter(difefencia >= 10L & difefencia <= 120L) %>%
 ggplot() +
  aes(x = difefencia, fill = fue_selc) +
  geom_histogram(bins = 30L) +
  scale_fill_manual(values = c(N = "#CF2216", Y = "#158CA9")) +
  scale_x_continuous(breaks = seq(10, 120, by = 10)) +  # <- más etiquetas en eje X
  labs(
    x = "Años de experiencia",
    y = "Numero",
    title = "Experiencia para el salon de la fama"
  ) +
  theme_minimal()


```
En esta gráfica podemos analizar a los candidatos que se postularon al Salón de la Fama.  
Observamos que los jugadores con menos de 20 años de experiencia en la liga fueron descartados en su gran mayoría. En contraste, los que tenían entre **22 y 27 años de experiencia** fueron quienes tuvieron mayor probabilidad de ser aceptados.

Esto podría deberse a que ese rango de años les da un margen adecuado para lograr buenas jugadas, destacar en su posición y posiblemente ayudar a su equipo a conseguir campeonatos.

También se observa que los jugadores nominados más de **70 años** después de haber iniciado su carrera fueron, en su mayoría, rechazados. Esto podría explicarse por la presencia de candidatos más jóvenes, con estadísticas recientes y pertenecientes a equipos más actuales, que llaman más la atención que los jugadores de generaciones pasadas.

Por lo tanto, conocer la **ventana de tiempo óptima** en la que un jugador es más probable que sea aceptado al Salón de la Fama, así como los **factores clave** que influyen en esa decisión, es esencial para entender cómo funciona el proceso de selección.
